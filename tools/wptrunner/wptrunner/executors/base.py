...
class RefTestImplementation(object):
    ...
    def get_differences(self, screenshots):
        diff = ImageChops.difference(Image.open(io.BytesIO(screenshots[0])),
                                     Image.open(io.BytesIO(screenshots[1])))
        ...
        count = stat.count[0]
        self.logger.info("Found %s pixels different, mzximum difference per channel %s" %
                         (count, per_channel))
